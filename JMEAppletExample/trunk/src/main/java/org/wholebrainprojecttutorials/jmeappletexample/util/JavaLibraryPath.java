package org.wholebrainprojecttutorials.jmeappletexample.util;
...
public class JavaLibraryPath {
	...
	public static void injectNativLib(LibraryElement... pathElements)
		...
		System.out.println("SYSTEM ARCHITECTURE: " + osAch);
		
		File path = null;

		for (LibraryElement pathElement : pathElements) {
			if (pathElement.matchesOsNameAndArch(osName)) {
				if (path == null) {
		...
		System.out.println("TOOK " + (end - start) + " miliseconds to copy dlls");
	}
	...
	private static File extract(String packageName, String... resourceNames)
			throws Exception {
		String tmpDirName = System.getProperty("user.home");
		...
		tmpDirName += File.separator + "wbc-natives";
